...
public class Chat implements javax.jms.MessageListener {
	...
	public static void main(String[] args) {
		try{
			...
			Chat chat = new Chat("jms/JNDITopic", "guest", "guest"); // Read from
																	// command
																	// line
			BufferedReader commandLine = new java.io.BufferedReader(
					new InputStreamReader(System.in)); // Loop until the word
														// "exit" is typed
			while (true) {
				String s = commandLine.readLine();
				if (s.equalsIgnoreCase("exit")) {
					chat.close(); // close down connection System.exit(0);//
									// exit program } else chat.writeMessage(s);
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
