<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=ISO-8859-2">
<title>
<c:choose>
	<c:when test="${param.ad_type == 'auto'}">
		Receipt for advertisement of ${auto.brand} ${auto.model}
	</c:when> <c:otherwise>
		Receipt for general ad ${genad.title}
	</c:otherwise>
</c:choose>
</title>
</head>
<body>
<div>
	<h3>Your advertisement details:</h3>
	You want to order an advertisement
	<c:choose> 
		<c:when test="${param.ad_type == 'auto'}">
			of ${auto.brand} ${auto.model}
		</c:when> <c:otherwise>
 			: ${genad.title}
		</c:otherwise>
	</c:choose>
	 <br>
	For ${param.months} month(s). <br>
	The price is ${price} € 
	<c:if test="${discount_used}">
		<div style='color: green;'>with ${discount_value}% of discount from your last transaction!</div>
	</c:if><br>
</div>
<div>
		<form action="index.jsp">
			<input type="hidden" name="price" value="${price}"> 
			<input type="hidden" name="id"
			value="<c:choose><c:when test="${param.ad_type == 'auto'}">${auto.adId}</c:when><c:otherwise>${genad.adId}</c:otherwise></c:choose>">
			<input type="hidden" name="ad_type" value="${param.ad_type}">
			<input type="hidden" name="page" value="payment.jsp">
			<input type="submit" value="Proceed to payment">
		</form>
		<form action="ReturnDiscountServlet">	
			<input type="hidden" name="discount_value" value ="${discount_value}">					
			<input type="submit" value="Cancel">
		</form>
	</div>
</body>
</html>